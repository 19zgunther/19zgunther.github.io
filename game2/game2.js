...
const P5EquipotentialSketch = p => {
    ...
    p.Update = function() {
        ...
        var startRadius = 20; //these two values cannot be above 20 for some reason or else the page freezes.
        var drawRadius = 20;
        ...
        for (var i=0; i<particles.length; i++) //for each particle
        {
            ...
            for (var j=0; j<equipotentials.length; j++) //for each equipotential
            {
                ...
                var dx = drawRadius*Math.cos(angleToCenter); 
                var dy = drawRadius*Math.sin(angleToCenter);
                safetyBrake = 0;
                if (particles[i].charge > 0) {
                    while (CalcLite(x,y) > targetVolt && safetyBrake < 500) //the way we decide if we found the particle, is if we go from being greater than the wanted voltage to less (and opposite for negative particle/charges)
                    {
                        x += dx;
                        y += dy;
                        safetyBrake += 1;
                    ...
                    while (CalcLite(x,y) < targetVolt && safetyBrake < 500)
                    {
                        x += dx;
                        y += dy;
                        safetyBrake += 1;
                ...
                if (safetyBrake > 495){ //if we went 495 steps or more, just stop there's no point in continuing because the equipotential is SOOOO far away (and likely completely off the screen)
                    continue;
                }
                ...
                for (var runs=0; runs<500; runs++) //for 500 runs we're going to run a greedy algorithm to draw the equiptoential 
                {
                    ...
                    currentVolt = CalcLite(nextX,nextY);                    //Finding the voltage at this new point
                    ...
                    while (currentVolt < targetVolt && safetyBrake < 200)      //if and while the voltage at the nextX and nextY point is too low, rotate a little bit (towards opposite direction of electric field vector)
                    {
                        ...
                        nextY = y + drawRadius * Math.cos(currentBestAngle);
                        currentVolt = CalcLite(nextX,nextY);
                    }
        ...
        var nx = 0; //"NEW" x
        var ny = 0; //"New" y
        var s; //this holds the data we get from the CalcMedium Function
        var t=0; //while loop failsafe counter (inner)
        var i=0; //while loop failsafe counter (outer)
        var drawLine = 5;
        var voltDrawThreshold = 1;
        var currentVolt = 1000000;
        var wantedVolt = 10;
        for(var equip in equipotentials) {
            wantedVolt = equipotentials[equip];
            voltageDrawThreshold = 0.1;

            p.stroke(Math.max((wantedVolt+8)*20,50), 50, Math.max((-wantedVolt+8)*20,50) );

            for (var q=0; q<particles.length; q++)
            {
                i=0;
                sx = nx = x = particles[q].posx+5;
                sy = ny = y = particles[q].posy+5;
                drawLine = 0;

                var rotation = Math.PI/2;
                var rotationChangeAmount = 0.01;
                if (q%2 == 0) {
                    
                }
                while(i < 500)
                {
                    i += 1;                         //while loop fail safe
                    s = CalcMedium(x,y);            //This gives us [voltage, angleOfElectricField] at (x,y)
                    angle = s[1] + rotation;       //rotating the angle by 90 degrees and updating variable "angle" from "s"
                    
                    t = 0; //this is a failsafe counter for the while loops to make sure they do not run forever.
                    currentVolt = CalcLite(x+radius*Math.sin(angle),y+radius*Math.cos(angle));
                    while (currentVolt < wantedVolt) //if the current voltage is too low, adjust angle accordingly
                    {
                        t += 1;
                        angle += rotationChangeAmount;
                        if (t >157)
                        { break;}
                        currentVolt = CalcLite(x+ radius*Math.sin(angle),y+radius*Math.cos(angle));
                    }
                    t = 0; //resetting failsafe
                    while (currentVolt > wantedVolt) //if the current voltage is too high, adjust angle accordingly
                    {
                        t += 1;
                        angle -= rotationChangeAmount;
                        if (t > 157)
                    ...
                    //We need to make sure drawLine is True and that the position to draw to is within the canvas
                    nx = x+radius*Math.sin(angle);
                    ny = y+radius*Math.cos(angle);
                    if (drawLine <= 0 && x < width && nx < width && x > 0 && nx > 0 && y < height && ny < height && y > 0 && ny > 0)
                    {
                        p.line(x,y, nx, ny);
                    } else if (drawLine < 5 && drawLine > 0)
                    {
                        drawLine -= 1;
                        if (drawLine == 0)
                        {
                    ...
                    x = nx; //updating x and y positions to the new position
                    y = ny;

                    //Deciding if we can start drawing a line, aka we're close enough to the equipotential line to start drawing and we should set our start positions sx sy now.
                    if (drawLine == 5 && voltDrawThreshold > Math.abs(currentVolt - wantedVolt))
                    {
                        TEST.innerHTML = "TRUE " + currentVolt;
                        drawLine = 4;
                        continue;
                    }

                    //Testing to see if we've completed a loop back to the sx sy point. If so, Break!                    
                    if (Math.abs(sx-x) < radius/4 && Math.abs(sy-y) < radius/1.1)
                    {
                        break;
                    }
                }
